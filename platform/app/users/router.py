...
async def test_create_user(
    ...
    try:
        await UsersDAO.add_user(username=username, email=email, password=get_password_hash(password))
        return redirect_message(url="/docs", message="Пользователь создан!", success=True)
    except IntegrityError:
        return redirect_message(url="/docs", message="Пользователь уже существует.", error=True)
    except SQLAlchemyError:
        return redirect_message(url="/docs", message="Ошибка БД.", error=True)
...
async def test_add_map(
    ...
    """Добавляет карту (данные захардкожены для теста)."""
    try:
        test_data = [[1, 0, 1], [0, 1, 0], [1, 0, 1]]
        await MapDAO.add_map(email=email, mapname=mapname, data=test_data)
        return redirect_message(url="/docs", message="Карта добавлена!", success=True)
    except ValueError:
        return redirect_message(url="/docs", message="Пользователь не найден.", error=True)
    except IntegrityError:
        return redirect_message(url="/docs", message="Карта с таким именем уже есть.", error=True)
    except SQLAlchemyError:
        return redirect_message(url="/docs", message="Ошибка БД.", error=True)
...
async def test_update_map(
    ...
    """Обновляет карту (данные захардкожены для теста)."""
    try:
        new_data = [[9, 9, 9], [9, 9, 9], [9, 9, 9]]
        await MapDAO.update_map(email=email, mapname=mapname, new_data=new_data)
        return redirect_message(url="/docs", message="Карта обновлена!", success=True)
    except ValueError:
        return redirect_message(url="/docs", message="Пользователь не найден.", error=True)
    except SQLAlchemyError:
        return redirect_message(url="/docs", message="Ошибка БД.", error=True)